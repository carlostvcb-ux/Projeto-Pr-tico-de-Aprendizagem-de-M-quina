...
O que na realidade queremos saber é: " É possível usar características quantitativas para predizer as notas dos alunos na disciplina de português e matemática (G3) face ao consumo de álcool?"

**4 Predição para as notas de matemática**

**4.1 Amostra de Entrada**

A tabela a seguir, nos mostra as primeiras seis linhas do banco de dados *maths.csv*.

```{r echo=FALSE, message=FALSE}
library(readr)
df_mat <- read_csv("/Users/user/Documents/ESTATISTICA/APRENDIZADO_DE_ MAQUINA/PROJETO_FINAL/PROJETO_APRENDIZADO_MAQUINA/maths.csv")
attach(df_mat)
```
...
set.seed(42)
df_mat
df_mat1 <- df_mat[,-c(31,32)]#Excluindo as variáveis G1 e G2
skim(df_mat1)
```

As variáveis G1 e G2 não são siguinificativas para o modelo e por isso foram retiradas do conjunto de dados.
Ao analisarmos o banco de dados, observamos que ele apresenta a informação que há trezentas e noventa e cinco observações e trinta e uma variáveis, sendo quatoze com a classificação numérica (*age,Medu,Fedu,traveltime,studytime,failures,famrel,freetime,goout,Dalc,Walc,health,absences e G3*) e dezessete do tipo character(*school,sex,address,famsize,Pstatus,Mjob,Fjob,reason,guardian,schoolsup,famsup,paid,activities,nursery,higher,internet e romantic*).

```{r echo=FALSE}
df_mat1 %>% DataExplorer::plot_intro()
```

O conjunto de dados é composto por 54,8% colunas discretas, 45,2% continuas, não contém dados faltantes (NAs) e 100,00% das linhas completas.
...
Podemos observar que as variáveis que mais possui uma correlação moderada com nossa variável alvo (G3) são: Medu e Fedu. Com uma correlação baixa temos a variavel Failures.
...
Obtemos trezentas e quatoze amostras para o nosso conjunto de treinamento e oitenta e uma amostras para o nosso conjunto de teste.
...
O teste do *best_results* informa e confirma a análise gráfica de que o melhor modelo é o *Preprocessor1_Model2*, ou seja, o *random_forest*.
...
Assim, obtemos que o melhor modelo (*random_forest*) apresentou um RMSE = 4,1934270 e um rsq = 0,2332309.
